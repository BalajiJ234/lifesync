...
const statusColors: Record<BucketStatus, { bg: string; text: string; border: string }> = {
  UNDER: {
...
export default function BudgetBucketCard({ bucket, onClick }: BudgetBucketCardProps) {
  const config = bucketConfig[bucket.type]
  const statusColor = statusColors[bucket.status]
  
  const percentUsed = bucket.planned.amount > 0 
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${config.bgGradient} flex items-center justify-center text-xl`}>
            {config.icon}
          </div>
          <div>
        ...
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColor.bg} ${statusColor.text}`}>
          {bucket.status === 'UNDER' ? 'On Track' : bucket.status === 'NEAR_LIMIT' ? 'Near Limit' : 'Over Budget'}
        </span>
      </div>
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className={`font-medium ${bucket.status === 'OVER' ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'}`}>
            {formatCurrency(bucket.spent.amount, bucket.spent.currency)}
          </span>
        </div>
        <div className="flex justify-between pt-2 border-t border-gray-100 dark:border-gray-700">
          <span className="text-gray-500 dark:text-gray-400">Remaining</span>
          <span className={`font-bold ${bucket.remaining.amount < 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
