...
export default function BudgetDashboard() {
  ...
  const { selectedMonth, currentPlan, allPlans, rules, insights, loading } =
    ...
    <div className='space-y-6'>
      ...
      {!activePlan && (
        <div className='bg-gray-50 dark:bg-gray-800 rounded-xl p-8 text-center'>
          ...
          <p className='mt-2 text-gray-500 dark:text-gray-400'>
            Create a budget plan to start tracking your spending with the{" "}
            {rules.buckets.needs}/{rules.buckets.wants}/{rules.buckets.savings}/
            {rules.buckets.debt} rule.
          </p>
          {totalMonthlyIncome > 0 ? (
            <p className='mt-2 text-sm text-green-600 dark:text-green-400'>
              Based on your income entries: {baseCurrency}{" "}
              {totalMonthlyIncome.toLocaleString()}
            </p>
          ) : (
            <p className='mt-2 text-sm text-amber-600 dark:text-amber-400'>
              No income entries found. Add income first or we&apos;ll use a
              default of {baseCurrency} 5,000.
            </p>
          )}
          <button
            onClick={handleCreatePlan}
      ...
      {activePlan && (
        <>
          ...
          <div className='bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6'>
            ...
            <div className='flex items-center justify-between'>
              ...
              <span className='text-2xl font-bold text-green-600 dark:text-green-400'>
                {activePlan.baseCurrency}{" "}
                {activePlan.totalIncome.amount.toLocaleString(undefined, {
          ...
          {/* Insights Panel */}
          {insights.length > 0 && <BudgetInsightsPanel insights={insights} />}
        </>
      )}
