...
export interface UserSettings {
  ...
  reportCurrency: string; // Currency for reports/analytics (can be any supported currency)
  useLiveExchangeRates: boolean; // Use live rates from API vs static rates
  language: "en" | "ar" | "hi";
...
const defaultSettings: UserSettings = {
  ...
  theme: "auto",
  currency: "AED",
  reportCurrency: "AED", // Default report currency same as main currency
  useLiveExchangeRates: true, // Enable live rates by default
  language: "en",
...
const settingsSlice = createSlice({
  ...
  reducers: {
    ...
    setReportCurrency: (state, action: PayloadAction<string>) => {
      state.settings.reportCurrency = action.payload;
    },
    setUseLiveExchangeRates: (state, action: PayloadAction<boolean>) => {
      state.settings.useLiveExchangeRates = action.payload;
    },
  },
});
...
export const selectDateFormat = (state: { settings: SettingsState }) =>
  state.settings.settings.dateFormat;
...
export const selectReportCurrency = (state: { settings: SettingsState }) =>
  state.settings.settings.reportCurrency;

export const selectUseLiveExchangeRates = (state: { settings: SettingsState }) =>
  state.settings.settings.useLiveExchangeRates;
