  ...
  switch (type) {
    case "spending":
      ...
      insights.push({
        title: "Budget Alert",
        description:
          "You're 80% through your monthly food budget with 10 days left",
        type: "alert",
        importance: "high",
    ...
    default:
      insights.push({
        ...
        description:
          "Start tracking your expenses and todos to get personalized insights",
        type: "welcome",
        importance: "low",
...
async function analyzeExpensePatterns(
  expenses: {
    category: string;
    amount: number;
    date: string;
  ...
  if (topCategory) {
    ...
    suggestions.push({
      ...
      message: `You spend most on ${category} ($${(amount as number).toFixed(
        2
      )})`,
  ...
  // Frequency analysis
  const dailySpending: Record<string, number> = expenses.reduce(
    (acc, expense) => {
  ...
  const averageDailySpend =
    Object.values(dailySpending).reduce((a: number, b: number) => a + b, 0) /
    Object.keys(dailySpending).length;
  const highSpendDays = Object.entries(dailySpending).filter(
    ([, amount]) => (amount as number) > averageDailySpend * 1.5
  );
...
async function suggestBillSplitting(data: {
  amount: number;
  description?: string;
}) {
  const suggestions = [];
  ...
  if (
    data.description?.toLowerCase().includes("dinner") ||
