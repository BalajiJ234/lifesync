...
function categorizeExpense(
  description: string,
  amount: number,
  context?: { date?: string; location?: string }
): CategoryResult {
  ...
  if (
    desc.match(
      /furniture|chair|table|sofa|bed|desk|cabinet|shelf|couch|mattress|lamp|decor|home goods|appliance|electronics|tv|computer|phone|gadget/
    )
  ) {
    return {
      category: "Shopping",
      confidence: 0.9,
      reason: "Contains furniture/electronics/home goods keywords",
    };
  }
  ...
  if (
    desc.match(
      /restaurant|food|lunch|dinner|breakfast|cafe|pizza|burger|sushi|takeout|delivery|eat|meal|dining|kitchen|cook|groceries|grocery|supermarket|market|carrefour|spinneys|lulu|union coop|fresh|produce|vegetables|fruits/
    )
  ) {
    return {
      category: "Food & Dining",
      confidence: 0.9,
      reason: "Contains food/dining/grocery keywords",
    };
  }
  ...
  if (
    desc.match(
      /gas|fuel|uber|lyft|taxi|bus|train|metro|parking|toll|car|vehicle|transport/
    )
  ) {
    return {
      category: "Transportation",
      confidence: 0.85,
      reason: "Contains transportation keywords",
    };
  }
  ...
  if (
    desc.match(
      /amazon|target|walmart|store|shop|buy|purchase|mall|retail|clothes|clothing|online|order/
    )
  ) {
    return {
      category: "Shopping",
      confidence: 0.8,
      reason: "Contains shopping keywords",
    };
  }
  ...
  if (
    desc.match(
      /electric|water|gas bill|internet|phone|cell|mobile|utility|bill|subscription|netflix|spotify/
    )
  ) {
    return {
      category: "Bills & Utilities",
      confidence: 0.95,
      reason: "Contains utility/subscription keywords",
    };
  }
  ...
  if (
    desc.match(
      /movie|cinema|theater|game|entertainment|fun|party|bar|club|concert|show/
    )
  ) {
    return {
  ...
  if (
    desc.match(
      /doctor|hospital|pharmacy|medical|health|dentist|clinic|medicine|drug|prescription/
    )
  ) {
    return {
      category: "Healthcare",
      confidence: 0.9,
      reason: "Contains healthcare keywords",
    };
  }
  ...
  if (
    desc.match(
      /rent|mortgage|housing|apartment|home|property|maintenance|repair/
    )
  ) {
    return {
      category: "Housing",
      confidence: 0.95,
      reason: "Contains housing keywords",
    };
  }
...
function calculateConfidence(description: string, category: string): number {
  ...
  const categoryPatterns: Record<string, RegExp[]> = {
    "Food & Dining": [
      /restaurant|food|lunch|dinner|breakfast|groceries|grocery|supermarket|carrefour|dining|kitchen|cook/,
    ],
    Transportation: [/gas|fuel|uber|taxi|parking|metro|bus/],
    Shopping: [
      /furniture|chair|table|sofa|bed|electronics|tv|computer|phone|amazon|store|shop|buy|purchase|mall|retail|clothes|appliance/,
    ],
  ...
  if (
    category === "Shopping" &&
    desc.match(/furniture|chair|table|sofa|bed|desk/)
  ) {
    confidence += 0.3;
  }
...
async function getSuggestions(
  ...
  if (
    ...
    suggestions.push({
      type: "recurring",
      message:
        "This looks like a recurring expense. Set up automatic tracking?",
  ...
  // Split suggestion
  if (
    description.toLowerCase().match(/dinner|lunch|meal|restaurant/) &&
    amount > 30
  ) {
    suggestions.push({
      type: "split",
