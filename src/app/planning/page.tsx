...
const goalCategories = [
  ...
  { value: 'vehicle', name: 'Vehicle', icon: Car, color: 'bg-gray-100 text-gray-800', description: 'Car purchase or maintenance' },
...
export default function BudgetGoalsPage() {
  ...
  const { selectedMonth, currentPlan, allPlans, insights, loading } = useAppSelector((state) => state.budget)
  ...
  const goalStats = useMemo(() => {
    if (goals.length === 0) return { total: 0, saved: 0, avgProgress: 0, activeCount: 0 }
  ...
  const analyzeGoalWithAI = async (goal: ReduxGoal) => {
    ...
    let recommendation = ''
    if (feasibility === 'easy') {
      recommendation = `Great! You can easily reach this goal by saving ${formatAmount(monthlyTarget, getCurrencyByCode(goal.currency))} per month.`
    } else if (feasibility === 'challenging') {
      recommendation = `This goal is ambitious! Consider extending the deadline or reducing expenses to free up ${formatAmount(monthlyTarget, getCurrencyByCode(goal.currency))}/month.`
    } else {
      recommendation = `This goal is achievable with discipline. Save ${formatAmount(monthlyTarget, getCurrencyByCode(goal.currency))} monthly.`
    }
  ...
  return (
    <div className="container mx-auto px-4 py-4 md:py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          onClick={activeTab === 'budget' ? handleCreatePlan : addGoalModal.openModal}
          disabled={activeTab === 'budget' && isCreating}
          className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50"
        >
          <Plus size={20} />
          <span>{activeTab === 'budget' ? 'Create Budget Plan' : 'Add Goal'}</span>
        </button>
      </div>
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-4 md:p-6 rounded-lg shadow-md">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold mt-1">{formatAmount(totalMonthlyIncome, currency)}</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 text-white p-4 md:p-6 rounded-lg shadow-md">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold mt-1">{formatAmount(goalStats.saved, currency)}</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 text-white p-4 md:p-6 rounded-lg shadow-md">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold mt-1">{goalStats.avgProgress.toFixed(0)}%</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-orange-500 to-orange-600 text-white p-4 md:p-6 rounded-lg shadow-md">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl md:text-2xl font-bold mt-1">{goalStats.activeCount}</p>
            </div>
        ...
        <div className="space-y-6">
          ...
          {!activePlan ? (
            <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
              ...
              <h3 className="text-xl font-medium text-gray-900 dark:text-white mb-2">No Budget Plan for {displayMonth}</h3>
              <p className="text-gray-600 dark:text-gray-400 mb-2">
                Monthly Income: {formatAmount(totalMonthlyIncome, currency)}
              </p>
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                Create a budget plan using the 50/30/20 rule to manage your spending.
              </p>
              <button
              ...
              <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    ...
                    <p className="text-xl font-bold text-blue-600">{formatAmount(activePlan.totalIncome.amount, currency)}</p>
                  </div>
                  <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400">Needs (50%)</p>
                    <p className="text-xl font-bold text-green-600">{formatAmount(activePlan.buckets.NEEDS.planned.amount, currency)}</p>
                  </div>
                  <div className="text-center p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400">Wants (30%)</p>
                    <p className="text-xl font-bold text-yellow-600">{formatAmount(activePlan.buckets.WANTS.planned.amount, currency)}</p>
                  </div>
                  <div className="text-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    <p className="text-sm text-gray-600 dark:text-gray-400">Savings (15%)</p>
                    <p className="text-xl font-bold text-purple-600">{formatAmount(activePlan.buckets.SAVINGS.planned.amount, currency)}</p>
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-6">
          {goals.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">Start tracking your financial goals with AI-powered insights!</p>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(goals as ReduxGoal[]).map((goal: ReduxGoal) => {
                ...
                return (
                  <div key={goal.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 hover:shadow-md transition-shadow">
                    {/* Goal Header */}
                    ...
                    <div className="px-6 pb-6 space-y-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                            const newAmount = prompt('Enter new saved amount:', goal.currentAmount.toString())
                            if (newAmount) updateGoalProgress(goal.id, parseFloat(newAmount))
                          }}
      ...
      >
        <GoalForm onSubmit={handleAddGoal} onCancel={addGoalModal.closeModal} currency={baseCurrency} />
      </ResponsiveModal>
      ...
      >
        {editingGoal && (
          <GoalForm onSubmit={handleEditGoal} onCancel={editGoalModal.closeModal} currency={baseCurrency} initialData={editingGoal} />
        )}
      </ResponsiveModal>
            ...
            {aiAnalysisModal.aiAnalysis && (
              <>
                <div className={`p-4 rounded-lg ${
                  aiAnalysisModal.aiAnalysis.feasibility === 'easy' ? 'bg-green-100 text-green-800' :
                  aiAnalysisModal.aiAnalysis.feasibility === 'challenging' ? 'bg-red-100 text-red-800' :
                  'bg-yellow-100 text-yellow-800'
                }`}>
                  <p className="font-medium">
                    Feasibility: {aiAnalysisModal.aiAnalysis.feasibility.charAt(0).toUpperCase() + aiAnalysisModal.aiAnalysis.feasibility.slice(1)}
                  </p>
                  <p className="text-sm mt-1">
                    Timeframe: {aiAnalysisModal.aiAnalysis.timeframe}
                  </p>
                  <p className="text-sm">
                    Monthly Required: {formatAmount(aiAnalysisModal.aiAnalysis.monthlyRequired, getCurrencyByCode(aiAnalysisModal.currency))}
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <ul className="list-disc pl-5 space-y-1 text-gray-600 dark:text-gray-400">
                    {aiAnalysisModal.aiAnalysis.suggestions.map((suggestion, i) => (
                ...
                <p className="text-sm text-gray-500">
                  Confidence: {(aiAnalysisModal.aiAnalysis.confidence * 100).toFixed(0)}%
                </p>
              </>
            )}
...
function GoalForm({ onSubmit, onCancel, currency, initialData }: GoalFormProps) {
